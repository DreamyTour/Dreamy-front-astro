      ...
      <div className="space-y-2">
        {content.map((block: any, blockIndex: number) => {
          if (block.type === "paragraph") {
            return (
              <p key={blockIndex} className="text-base leading-relaxed text-gray-600">
          ...
          if (block.type === "heading") {
            ...
            const headingText = (block.children || []).map((child: any, i: number) => child.text || "").join("");
            
            if (level === 1) {
              return <h1 key={blockIndex} className="text-xl font-bold text-gray-900">{headingText}</h1>;
            }
            if (level === 2) {
              return <h2 key={blockIndex} className="text-lg font-bold text-gray-900">{headingText}</h2>;
            }
            return <h3 key={blockIndex} className="text-base font-semibold text-gray-800">{headingText}</h3>;
          }
          ...
          if (block.type === "list") {
            ...
            return (
              <ListTag key={blockIndex} className={`${format === "ordered" ? "list-decimal" : "list-disc"} ml-4 text-base text-gray-600 space-y-1`}>
                {(block.children || []).map((listItem: any, i: number) => (
                  <li key={i} className="text-gray-600">
                    {(listItem.children || []).map((child: any, j: number) => child.text || "").join("")}
                  </li>
                ))}
              </ListTag>
            );
