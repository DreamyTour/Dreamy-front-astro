...
export default function LibroReclamaciones() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert('Su reclamación ha sido enviada correctamente. Nos pondremos en contacto en un plazo máximo de 30 días hábiles.');
      } else {
      ...
      alert('Hubo un error al enviar su reclamación. Por favor intente nuevamente o contáctenos directamente.');
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              ...
              )).sort((a: any, b: any) => a.props.children.localeCompare(b.props.children))}
            </select>
          </div>

          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: colors.foreground }}>
              N° Documento (DNI / Pasaporte) <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              name="documento"
          ...
          <div>
            ...
            <div className="flex gap-4 mt-2">
              <label className="inline-flex items-center cursor-pointer">
                ...
                />
                <span className="ml-2" style={{ color: colors.foreground }}>No</span>
              </label>
              <label className="inline-flex items-center cursor-pointer">
                <input
                  type="radio"
                  ...
                  style={{ accentColor: colors.primary }}
                />
                <span className="ml-2" style={{ color: colors.foreground }}>Sí</span>
              </label>
            </div>
          </div>

          <div className={formData.menorEdad === 'si' ? 'md:col-span-2' : 'hidden'}>
            <label className="block text-sm font-medium mb-2" style={{ color: colors.foreground }}>
              Datos del Padre, Madre o Apoderado <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              name="datosApoderado"
      ...
      <div>
        <h3 className="text-lg font-semibold mb-4 pt-4 border-t" style={{ color: colors.foreground, borderColor: colors.border }}>
          MANIFIESTO DEL CONSUMIDOR RECLAMANTE
        </h3>
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex gap-6 mt-2">
              <label
                ...
                style={{
                  borderColor: formData.tipo === 'reclamo' ? colors.primary : colors.border,
                  backgroundColor: formData.tipo === 'reclamo' ? `${colors.primary}10` : 'transparent',
                }}
              >
                <input
                  type="radio"
                  ...
                  style={{ accentColor: colors.primary }}
                />
                <span className="ml-2 font-medium" style={{ color: colors.foreground }}>Reclamo</span>
              </label>
              <label
                className="inline-flex items-center p-4 border rounded-lg cursor-pointer transition-all"
                style={{
                  borderColor: formData.tipo === 'queja' ? colors.primary : colors.border,
                  backgroundColor: formData.tipo === 'queja' ? `${colors.primary}10` : 'transparent',
                }}
              >
                <input
                  type="radio"
                  ...
                  style={{ accentColor: colors.primary }}
                />
                <span className="ml-2 font-medium" style={{ color: colors.foreground }}>Queja</span>
              </label>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: colors.foreground }}>
              Detalle de la Reclamación o Queja <span className="text-red-500">*</span>
            </label>
            <textarea
              name="detalle"
      ...
      <div className="mt-8">
        ...
        >
          {isSubmitting ? '⏳ Enviando...' : submitStatus === 'success' ? '✅ Enviado exitosamente' : submitStatus === 'error' ? '❌ Error al enviar' : 'Enviar'}
        </button>
      </div>
    </form>
  );
